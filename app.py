    ...
    if st.sidebar.button("Send"):
        if user_input:
            try:
                ...
                answer = response.choices[0].message.get("content", "No response received.")
                st.sidebar.markdown(f"**AI Assistant:** {answer}")
            except Exception as e:
                st.sidebar.error(f"Error contacting OpenAI: {e}")
...
if page == "Home":
    st.markdown("""
    <style>
    .main-title {font-family: 'Arial', sans-serif; font-size: 42px; color: #2E86C1; text-align: center; margin-bottom: 5px;}
    .sub-title {font-family: 'Arial', sans-serif; font-size: 20px; color: gray; text-align: center; margin-top: -5px; margin-bottom: 20px;}
    </style>
    ...
    st.markdown("<div style='text-align:center; margin-top:20px;'><p style='font-size:18px; color:#555;'>Navigate through modules using the sidebar. Ask the AI assistant for advice anytime!</p></div>", unsafe_allow_html=True)
